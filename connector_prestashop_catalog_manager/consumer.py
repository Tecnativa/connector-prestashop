...
def _get_shop_location(session):
    url = session.context.get('shop_url', None)
    res = {
        'location': url,
    }
    return res
    ...
    if set(fields.keys()) <= set(CATEGORY_EXPORT_FIELDS):
        ...
        for binding in record.prestashop_bind_ids:
            export_record.delay(
                ...
                priority=20, connector_env_options=_get_shop_location(session))
...
def product_product_write(session, model_name, record_id, fields):
    ...
    if 'active' in fields and not fields['active']:
        prestashop_product_combination_unlink(
            session, record_id,
            connector_env_options=_get_shop_location(session))
        return
